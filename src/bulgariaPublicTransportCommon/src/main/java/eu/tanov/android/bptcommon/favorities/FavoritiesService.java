package eu.tanov.android.bptcommon.favorities;
...
public class FavoritiesService {
    ...
    private final int favorities_null_label;
	...
	public FavoritiesService(Context context, int favorities_null_label) {
		this.context = context;
		this.favorities_null_label = favorities_null_label;
	...
	private void fixProviders() {
        ...
        for (Entry<String, ?> next : positionsStore.getAll().entrySet()) {
            positionsStoreEditor.putString(next.getKey(), mergeProviderAndCode(PROVIDER_SOFIATRAFFIC, (String) next.getValue()));
        }
        for (Entry<String, ?> next : labelsStore.getAll().entrySet()) {
            labelsStoreEditor.putString(mergeProviderAndCode(PROVIDER_SOFIATRAFFIC, next.getKey()),
	...
	private BusStopItem busStopFromEntry(Entry<String, ?> positionEntry, Map<String, ?> allLabels) {
		...
		if (label == null || EMPTY_LABEL.equals(label)) {
		    label = context.getString(favorities_null_label);
		}
